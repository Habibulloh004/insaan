    ...
    <main className="relative min-h-[250px] md:h-[calc(100vh-80px)]">
      ...
      <section className="max-w-[1440px] w-11/12 mx-auto relative h-full py-10">
        ...
        <div className="w-2/3 md:w-1/2 relative z-10 h-full flex justify-center items-start flex-col gap-4 text-black">
          <h1
            className={`textBig3 max-sm:text-xl max-md:text-4xl font-bold text-${textColor}`}
          >
            {title}
          </h1>
          <p
            className={`textNormal4 max-md:text-md max-sm:text-sm text-${textColor}`}
          >
            {description}
          </p>
        </div>
      ...
      {showCard && (
        <div
          className={`z-20 bg-white/80 absolute ${getCardPositionClasses()} rounded-md p-3 md:p-6 text-primary flex text-center items-center flex-col gap-2`}
        >
          <h1 className="text-center textSmall3 w-[120px] md:w-[250px]">
            {cardTitle}
          </h1>
          <Link prefetch={true} href={cardButtonLink}>
